...
(function (global, chrome, document) {
  class Provider {
    ...
    createTag (id, host, env, elementId, attr, status) {
      ...
      return `"use strict";(function(e,t,r,n){r=t.getElementById("${elementId}");if(e.UDTracker||e.USERDIVEObject){r.setAttribute("${attr}","used")}else{(function(e,t,r,n,c,a,i,s){e.USERDIVEObject=c;e[c]=e[c]||function(){(e[c].queue=e[c].queue||[]).push(arguments)};i=t.createElement(r);s=t.getElementsByTagName(r)[0];i.async=1;i.src=n;i.charset=a;s.parentNode.insertBefore(i,s)})(window,t,"script","//${host}/static/UDTracker.js?"+(new Date).getTime(),"ud","UTF-8");e.ud("create","${id}",{env:"${env}",cookieExpires:1});e.ud("analyze")}setTimeout(function(){if(!e.UDTracker){console.warn("Blocked USERDIVE Scripts");return}try{n=e.UDTracker.cookie.fetch();var t=["pageId","visitorType"];var c="";for(var a in t){c+=a<t.length-1?t[a]+":"+n[t[a]]+",":t[a]+":"+n[t[a]]}r.setAttribute("${status}",c)}catch(i){r.setAttribute("${attr}","err")}},2e3)})(window,document);`;
    }
    load () {
